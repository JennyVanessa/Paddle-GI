...
def main():
    ...
    try:  # for unexpected error logging
        ...
        if is_image_file(args.image):
            ...
            mod=paddle.load("/home/aistudio/generative-inpainting/checkpoints/imagenet/hole_benchmark/gen_00060000.pdparams")
            netG.set_state_dict(mod)
            model_iteration = int(60000)
            ...
            print(PSNR)
            print(tvloss(inpainted_result))

            #vutils.save_image(inpainted_result, args.output, padding=0, normalize=True)
            cv2.imwrite(args.output,(paddle.transpose(inpainted_result[0],[1,2,0]).numpy()+1)*256)
